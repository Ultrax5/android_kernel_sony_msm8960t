...
#include "cputimehack.h"
#include "symsearch.h"

SYMSEARCH_DECLARE_FUNCTION_STATIC(void, cpufreq_notify_utilization_k, struct cpufreq_policy *policy, unsigned int util);
SYMSEARCH_DECLARE_FUNCTION_STATIC(void, unlock_policy_rwsem_write_k, int cpu);
SYMSEARCH_DECLARE_FUNCTION_STATIC(int, lock_policy_rwsem_write_k, int cpu);
	...
	if (!bypass) {
		if (reenable_timer) {
			...
			for_each_online_cpu(cpu) {
				if (lock_policy_rwsem_write_k(cpu) < 0)
					continue;
...
skip_this_cpu:
				unlock_policy_rwsem_write_k(cpu);
			}
		}
...
static void bds_check_cpu(struct cpu_bds_info_s *this_bds_info)
{
	...
	cpufreq_notify_utilization_k(policy, load_at_max_freq);
...
static int __init cpufreq_gov_bds_init(void)
{
	...
	unsigned int i;
	int cpu = get_cpu();
	SYMSEARCH_BIND_FUNCTION_TO(cpufreq_badass, cpufreq_notify_utilization, cpufreq_notify_utilization_k);
	SYMSEARCH_BIND_FUNCTION_TO(cpufreq_badass, unlock_policy_rwsem_write, unlock_policy_rwsem_write_k);
	SYMSEARCH_BIND_FUNCTION_TO(cpufreq_badass, lock_policy_rwsem_write, lock_policy_rwsem_write_k);
