...
SYMSEARCH_DECLARE_FUNCTION_STATIC(void, cpufreq_notify_utilization_k, struct cpufreq_policy *policy, unsigned int load);
SYMSEARCH_DECLARE_FUNCTION_STATIC(unsigned long, nr_running_k, void);
...
static void cpufreq_smartass_freq_change_time_work(struct work_struct *work)
{
	...
	for_each_possible_cpu(cpu) {
		...
		cpufreq_notify_utilization_k(policy,
...
static int __init cpufreq_smartass_init(void)
{
	unsigned int i;
	struct smartass_info_s *this_smartass;
	SYMSEARCH_BIND_FUNCTION_TO(cpufreq_smartassH3, cpufreq_notify_utilization, cpufreq_notify_utilization_k);
	SYMSEARCH_BIND_FUNCTION_TO(cpufreq_smartassH3, nr_running, nr_running_k);
	debug_mask = 0;
